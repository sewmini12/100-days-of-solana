...
var getBase58Encoder = () => getBaseXEncoder(alphabet2$1);
...
function getConstantDecoder(constant) {
	return createDecoder({
		fixedSize: constant.length,
		read: (bytes, offset) => {
			const base16 = getBase16Decoder$1();
...
var getBase58Encoder$1 = () => getBaseXEncoder$1(alphabet2);
var getBase58Decoder$1 = () => getBaseXDecoder$1(alphabet2);
function getLifetimeTokenEncoder() {
	return transformEncoder(getNullableEncoder(fixEncoderSize(getBase58Encoder$1(), 32), {
		noneValue: "zeroes",
		prefix: null
	}), (token) => token ?? null);
}
function getLifetimeTokenDecoder() {
	return fixDecoderSize(getBase58Decoder$1(), 32);
}
...
async function traverseSequential2(instructionPlan, context) {
	...
	if (context.parent && (context.parent.kind === "parallel" || !instructionPlan.divisible)) {
		if (await selectAndMutateCandidate(context, context.parentCandidates, (message) => fitEntirePlanInsideMessage(instructionPlan, message))) return null;
...
async function traverseMessagePacker(instructionPlan, context) {
	...
	const transactionPlans = [];
	const candidates = [...context.parentCandidates];
	while (!messagePacker.done()) if (!await selectAndMutateCandidate(context, candidates, messagePacker.packMessageToCapacity)) {
		const newPlan = {
			kind: "single",
			message: await createNewMessage(context, messagePacker.packMessageToCapacity),
			planType: "transactionPlan"
		};
...
async function selectAndMutateCandidate(context, candidates, predicate) {
	for (const candidate of candidates) try {
		...
		if (isSolanaError(error, 7618e3) || isSolanaError(error, 5663033) || isSolanaError(error, 5663035) || isSolanaError(error, 5663034) || isSolanaError(error, 5663032));
		else throw error;
	}
	return null;
}
...
function fitEntirePlanInsideMessage(instructionPlan, message) {
	...
	switch (kind) {
		case "sequential":
		case "parallel":
			for (const plan of instructionPlan.plans) newMessage = fitEntirePlanInsideMessage(plan, newMessage);
			return newMessage;
		case "single":
		...
		case "messagePacker":
			const messagePacker = instructionPlan.getMessagePacker();
			while (!messagePacker.done()) newMessage = messagePacker.packMessageToCapacity(newMessage);
			return newMessage;
		default: throw new SolanaError(SOLANA_ERROR__INVARIANT_VIOLATION__INVALID_INSTRUCTION_PLAN_KIND, { kind });
	}
}
		...
		if (signal) {
			...
			return await new Promise((resolve, reject) => {
				const handleAbort = (e) => {
					...
					queueMicrotask(() => {
						if (coalescedRequest.numConsumers === 0) coalescedRequest.abortController.abort(EXPLICIT_ABORT_TOKEN$1 ||= createExplicitAbortToken$1());
					});
...
function getMinimumBalanceForRentExemption(space) {
	...
	return (RENT.ACCOUNT_STORAGE_OVERHEAD + space) * RENT.DEFAULT_LAMPORTS_PER_BYTE_YEAR * RENT.DEFAULT_EXEMPTION_THRESHOLD;
}
